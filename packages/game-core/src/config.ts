 ...
 *
 * D-059: D-041のfloor(1200ms)でもまだ緩く、micro中心のeasyだと頭打ち後の
 * ペースを事実上いつまでも捌けてしまうと判明した。「一定値で頭打ちする」
 * 設計自体をやめ、floorを技術的な下限(50ms)まで引き下げることで、
 * 頭打ち後も実質的には誰にも捌けないペースまで際限なく加速するようにした。
 *
...
export const DEFAULT_CONFIG: GameConfig = {
  ...
  survivalRise: {
    ...
    // D-041で2400ms→1200msへ引き下げたが、micro中心のeasyだと1200msでも
    // 頭打ち後は事実上無限に生存できてしまうと判明した(D-059)。
    // 「一定値で頭打ちする」設計そのものをやめ、技術的な下限(0除算・
