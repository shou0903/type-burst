...
export class SoundEngine {
   ...
   * ミスキー(D-089で強化)。
   *
   * 元は triangle 150Hz / 80ms / gain 0.05 と非常に小さく、打鍵音に埋もれて
   * ミスに気付けなかった。他の効果音は元のまま維持し、この音だけを次の3点で
   * はっきりさせている:
   *  1. のこぎり波にして倍音を増やし「ブッ」という濁りを作る(三角波は柔らかすぎた)
   *  2. 音量を約3倍に上げ、打鍵音(gain 0.035の矩形波)に埋もれないようにする
   *  3. 明確に下降させ、上昇形の正解音・ロック音と方向で区別する
   * 基音は220Hz付近に置き、打鍵音(1150Hz超)とは音域そのものを分けている。
   * 連打されても痛くないよう、長さは150ms程度に抑える。
   */
  keyMiss(): void {
    this.tone(220, 150, { type: "sawtooth", gain: 0.15, endFreq: 110 });
    this.tone(165, 160, { type: "sawtooth", gain: 0.09, endFreq: 90, delayMs: 20 });
  }
